...
class DepartmentTemplateView(Model, AuditMixinNullable):

    __tablename__ = 'department'
    id = Column(Integer, primary_key=True)
    depart_name = Column(String(250))
    # demand_id = relationship('DepartmentTemplateView', secondary=depart_name)

    def __repr__(self):
        return self.depart_name


demand_user = Table('demand_user', metadata,
                    Column('id', Integer, primary_key=True),
                    Column('user_id', Integer, ForeignKey('ab_user.id')),
                    Column('demand_id', Integer, ForeignKey('demand_templates.id')),
                    )

handle_user = Table('handle_user', metadata,
                    Column('id', Integer, primary_key=True),
                    Column('user_id', Integer, ForeignKey('ab_user.id')),
                    Column('demand_id', Integer, ForeignKey('demand_templates.id')),
                    )

demand_status_sign = Table('demand_status_sign', metadata,
                    Column('id', Integer, primary_key=True),
                    Column('demand_status_id', Integer, ForeignKey('demand_status.id')),
                    Column('demand_id', Integer, ForeignKey('demand_templates.id'))
                    )

depart_name = Table('depart_name', metadata,
                    Column('id', Integer, primary_key=True),
...
class DemandStatus(Model, AuditMixinNullable):

    __tablename__ = 'demand_status'
    id = Column(Integer, primary_key=True)
...
class DemandTemplate(Model, AuditMixinNullable):

    __tablename__ = 'demand_templates'
    id = Column(Integer, primary_key=True)
    title = Column(String(250))
    category = relationship('DepartmentTemplateView', secondary=depart_name)
    field_list = Column(Text, default='')
    desc_text = Column(Text, default='')
    attach_path = Column(String(250))
    author = relationship(sm.user_model, secondary=demand_user)
    create_time = Column(DateTime)
    last_edit_time = Column(DateTime)
    handle_begin_time = Column(DateTime)
    handle_end_time = Column(DateTime)
    handler = relationship(sm.user_model, secondary=handle_user)
    status = relationship('DemandStatus', secondary=demand_status_sign)
    # depart_id = Column(Integer, ForeignKey('department_templates.id'))

DemandStatus.__table__.create(db.engine, checkfirst=True)
DepartmentTemplateView.__table__.create(db.engine, checkfirst=True)
DemandTemplate.__table__.create(db.engine, checkfirst=True)


demand_user.create(db.engine, checkfirst=True)
handle_user.create(db.engine, checkfirst=True)
demand_status_sign.create(db.engine, checkfirst=True)
depart_name.create(db.engine, checkfirst=True)
